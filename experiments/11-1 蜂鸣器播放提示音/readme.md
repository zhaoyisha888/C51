...
Hz（赫兹）= 每秒的周期数。
所以 **500Hz 的声音 = 每秒钟蜂鸣器的电压波形来回振荡 500 次**。
...
1. “**初始时间设置**”，就是定时器的初值（**TH0 / TL0**）。
2. **定时器溢出间隔 = 中断周期**。
3. **中断里翻转一次电平 → 就决定了蜂鸣器方波的半周期**。

**举例：**
设定的 TH0=0xFC, TL0=0x66 → 定时器每 1ms 中断一次。

中断里翻转蜂鸣器一次 → 半周期 = 1ms → 完整周期 = 2ms → 频率 = 500Hz。

>如果改成别的初值：
比如中断周期 = 0.5ms → 翻转一次电平时间更短 → 方波频率 = 1000Hz → 声音更尖。
比如中断周期 = 2ms → 方波频率 = 250Hz → 声音更低。

所以：**改定时器初值 = 改变了中断频率 = 改变了蜂鸣器方波频率 = 改变了音调。**



## 如何处理曲谱
![alt text](../../source/小星星简谱.jpg)
...
1. 没有特殊标记的数字（例如 1 1 5 5），都是 四分音符，即 1拍。
2. 数字后面带 横线 “–” 的，表示要 延长，这里通常是 二分音符（2拍）。

```ini
1 1 5 5 → 四分音符，每个音1拍。
6 6 5 - → 最后那个 5 - 就是二分音符，要持续2拍。
```

### 节奏时长怎么算（以程序为例）
**节拍速度（BPM，beats per minute）决定了“一拍”要持续多久**：
